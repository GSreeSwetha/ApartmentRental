...
begin
    ...
    select count(1) INTO l_cnt FROM users WHERE  id = listingId;
if(l_cnt >0) then
    if(usr_cnt =1) then  
...
else dbms_output.put_line('Invalid Listing Id');
end if;
Exception 
...
if(usr_cnt =1 and l_cnt = 1) then 
    INSERTDATA.INSERT_CONCERNS(sysdate,null,listingId,comments,'C',null,null,uid); --
...
begin
    ...
    SELECT count(1) INTO l_cnt FROM listing WHERE id = listingId;
if(usr_cnt =1 and l_cnt =1) then 
    INSERTDATA.INSERT_CONCERNS(sysdate,null,listingId,comments,'Q',null,null,uid); 
...
--if tag already exists in tag table
    if(tags_cnt >0) then
        ...
        select count(1) into flag_check from listing_tags where tag_id = tagId and listing_id = listingId; --if listing already has that tag
...
else dbms_output.Put_line('Given listing does not exist');
end if;
exception
  when others then
             dbms_output.Put_line(SQLERRM);
